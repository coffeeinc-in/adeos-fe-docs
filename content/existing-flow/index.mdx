...
This section documents the complete ADEOS FE (Annotation Detection and Extraction Optimization System) architecture, covering both frontend and backend processing flows. ADEOS FE is a real-time annotation processing system built with Next.js 15 and FastAPI, utilizing WebSocket for live updates and Redis for queue management.
...
### Batch Processing
Frontend submits annotation requests via `/api/process/process-annotation-ws`. Backend creates a batch job and pushes tasks to Redis queue.

### Worker Processing
Extraction workers (text, table, diagram) process annotations concurrently using AI models.
...
- **Worker Pools**: Horizontal scaling of extraction workers
...
- [Redis Integration](/sprint-02/redis) - Queue management and worker patterns