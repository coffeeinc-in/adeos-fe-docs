...
1. **Request**: Frontend sends annotation processing request
2. **Batch Creation**: Backend creates job document in MongoDB and initializes Redis progress tracker
3. **Task Distribution**: All annotations queued to Celery simultaneously
4. **Parallel Processing**: Celery automatically distributes tasks across 8 workers
5. **Progress Tracking**: Each worker atomically updates Redis counter after completion
6. **Completion Detection**: Last worker to finish detects all complete and removes batch from Redis
7. **Real-time Updates**: Pusher events keep frontend synchronized throughout
...
- **Celery App Configuration**: `backend/celery_app.py:1-50`
- **Process Annotation Task**: `backend/tasks/process_annotation.py:30-250`
- **Layout Detection**: `backend/services/layout_detection.py:10-100`
- **AI Extractors**: `backend/services/ai_extractors.py:10-150`
- **Blob Service**: `backend/services/azure_blob.py:20-80`
- **Redis Client**: `backend/services/redis_client.py:1-30`
